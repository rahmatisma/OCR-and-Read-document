...
def parse_spk_aktivasi(all_text: str, page_texts: list[str], ttd_results: dict = None, doc_results: dict = None) -> dict:
    """
    ...
    """
    parser = SPKAktivasiParser(all_text, page_texts, ttd_results, doc_results)
    return parser.parse()


class SPKAktivasiParser(BaseParser):
    """
    ...
    def _parse_jaringan_section(self, data: dict):
        ...
        jaringan["jenis_aktivasi"] = self.search_regex(
            r"Jenis\s*Aktivasi\s*:\s*([^\n]*)",
            self.all_text
        )
    ...
    def _parse_berita_acara_section(self, data: dict):
        ...
        berita_acara["tipe_spk"] = "aktivasi"
        ...
        berita_acara["jenis_aktivasi"] = self.search_regex(
            r"Jenis\s*Aktivasi\s*:\s*([^\n]*)",
            self.all_text
        )
