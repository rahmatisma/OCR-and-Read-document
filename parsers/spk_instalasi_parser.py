...
def parse_spk_instalasi(all_text: str, page_texts: list[str]) -> dict:
    ...
    data["spk"]["tipe_spk"] = "spk instalasi"
    ...
    data["jaringan"]["kecepatan"] = search_regex(r"Kecepatan\s*:\s*Downstream\s*/\s*Upstream\s*([^\n]*)", all_text)
    ...
    data["vendor"]["nama_teknisi"] = search_regex(r"Pelaksana\s*Survey.*?([A-Za-z\s]+)", all_text)
    ...
    data["informasi_gedung"]["pemilik_bangunan"] = search_regex(r"Pemilik\s*Bangunan\s*([^\n]*)", all_text)
...
def search_regex(pattern: str, text: str):
    ...
    if not match:
        return ""  # kosong string agar aman untuk DB
