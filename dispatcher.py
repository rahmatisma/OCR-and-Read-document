...
def detect_spk_type(all_text: str) -> str:
    ...
    # ❌ HAPUS INI - maintenance_remote_wireless tidak dipakai
...
def dispatch_parser(all_text: str, page_texts: list, ocr_data: list = None) -> dict:
    ...
    Args:
        all_text: Teks lengkap dari PDF
        page_texts: List teks per halaman
        ocr_data: List of OCR data dengan bbox/position
        
    Returns:
        Dictionary dengan format:
        {
            "document_type": "spk_survey",  # ← PENTING: untuk validasi Laravel
            "jenis_spk": "survey",
            "data": { ... parsed data ... },
            "metadata": { ... }
        }
    ...
    if parser_func:
        ...
        return {
            ...
            "metadata": {
                ...
                "note": "Jenis SPK tidak terdeteksi atau parser belum tersedia."
            }
        }


def get_available_spk_types() -> list:
    """Daftar jenis SPK yang didukung"""
    return [
        "spk_survey",
        "spk_instalasi",
        "spk_dismantle",
